...
字段说明：
- `type`：实体真实类型，不会返回独立的“别名”类型。
- `from_alias`：仅在远端别名候选命中时出现，值为实际命中的别名；正式名称命中不返回此字段。目前适用于干员、材料和敌人。
...
获取明日方舟游戏术语的简要解释，并级联返回解释中引用的相关机制。

### 参数

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `glossary_name` | `str` \| `list[str]` | 是 | 查询关键词；可以是单个字符串、逗号/顿号分隔的字符串、或字符串数组。优先匹配术语名称，名称未命中时匹配解释文本 |
