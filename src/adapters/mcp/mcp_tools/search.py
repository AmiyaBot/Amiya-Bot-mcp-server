...
# 升级为资源统一搜索（干员 + 召唤物 + 皮肤 + 材料 + 关卡 + 敌人），作为任何查询的统一入口。
...
def register_search_tool(mcp, app):
    ...
    async def search(
        query: Annotated[str, Field(description="搜索关键词（干员、召唤物、皮肤、材料、关卡或敌人名称/代号），支持模糊搜索")],
    ) -> dict:
        tool_name = "search"
        ...
        try:
            ...
            if repo is not None:
                try:
                    ...
                    bundle = None
                logger.debug(
                    "search 调用上下文: repo_ready=%s bundle_operators=%s name_index=%s token_name_index=%s enemy_alias_index=%s",
                    repo.is_ready(),
