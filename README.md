...
下面的命令会将 1.0.4 安装到 `~/.local/share/amiyabot-cli/venv`，并在 `~/.local/bin` 创建 `amiyabot-cli`：

```bash
curl -fsSL https://raw.githubusercontent.com/AmiyaBot/Amiya-Bot-mcp-server/v1.0.4/install.sh \
  | AMIYABOT_PIP_SOURCE="git+https://github.com/AmiyaBot/Amiya-Bot-mcp-server.git@v1.0.4" sh
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/AmiyaBot/Amiya-Bot-mcp-server/v1.0.4/install.sh \
  | AMIYABOT_PIP_SOURCE="git+https://github.com/AmiyaBot/Amiya-Bot-mcp-server.git@v1.0.4" sh -s -- --no-playwright
```

#### 从源码安装

```bash
git clone --branch v1.0.4 --depth 1 https://github.com/AmiyaBot/Amiya-Bot-mcp-server.git
